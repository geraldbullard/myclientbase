...
<div class="grid_10" id="content_wrapper">
		...
		<div class="content toggle">
			...
			<form method="post" action="<?php echo site_url($this->uri->uri_string()); ?>" name="invoice_item_form">
				...
				<dl>
					...
					<dd><input class="datepicker" type="text" name="item_date" value="<?php echo $this->mdl_items->form_value('item_date'); ?>" /></dd>
				</dl>
    ...
    <dl>
        <dt><label><?php echo $this->lang->line('save_as_stored_item'); ?>: </label></dt>
        <dd><input type="checkbox" name="save_as_stored_item" value="1" <?php if ($this->input->post('save_as_stored_item')) { ?>checked="checked<?php } ?>"/></dd>
    </dl>
				...
				<dl>
					...
					<dd>
						<select name="tax_rate_id">
							...
							<option value="<?php echo $tax_rate->tax_rate_id; ?>" <?php if(($this->mdl_items->form_value('tax_rate_id') and $this->mdl_items->form_value('tax_rate_id') == $tax_rate->tax_rate_id) or (!$this->mdl_items->form_value('tax_rate_id') and $this->mdl_mcb_data->setting('default_item_tax_rate_id') == $tax_rate->tax_rate_id)) { ?>selected="selected"<?php } ?>><?php echo $tax_rate->tax_rate_percent . '% - ' . $tax_rate->tax_rate_name; ?></option>
							<?php } ?>
						</select>
					</dd>
				</dl>

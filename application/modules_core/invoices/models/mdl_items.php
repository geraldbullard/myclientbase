...
class Mdl_Items extends MY_Model {
	...
	public function save($db_array, $invoice_item_id = NULL) {
        ...
        if ($this->input->post('save_as_stored_item')) {

            $db_array = array(
                'invoice_stored_item'               =>  $this->input->post('item_name'),
                'invoice_stored_unit_price'         =>  $db_array['item_price'],
                'invoice_stored_item_description'   =>  $this->input->post('item_description')
            );

            $this->db->insert('mcb_invoice_stored_items', $db_array);

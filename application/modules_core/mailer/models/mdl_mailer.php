...
class Mdl_Mailer extends MY_Model {
    ...
    public function email_invoice($invoice, $invoice_template, $from_email, $from_name, $to, $subject, $email_body, $invoice_as_body, $email_cc = NULL, $email_bcc = NULL) {
        ...
        $html = $this->load->view('invoices/invoice_templates/' . $invoice_template, $data, TRUE);

        pdf_create($html, $filename, FALSE);

        $this->load->helper('mailer/phpmailer');

        $email_body = ($invoice_as_body) ? nl2br($email_body) . $html : nl2br($email_body);
